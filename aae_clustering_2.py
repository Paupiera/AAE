...
def clustering_aae_2(latents_path,dataset):
    time_0=time.time()
    #CLUSTER 
    ...
    path_clu = 'Clusters_2'+latents_path.split('Latents')[1].split('latents.npz')[0]+'new_clusters.tsv'

    latent = vamb.vambtools.read_npz(latents_path)
    # Load aae the mask
    mask = vamb.vambtools.read_npz(mask_path)
    print('\n\n Latents Path: ',latents_path,'\n Mask path: ',mask_path,'\n Clusterrs path: ',path_clu,'\n')
    ...
    cluster_iterator = vamb.cluster.cluster(latent, labels=np.array(contignames)[mask],cuda=Cuda) # Iterative medoid cluster generator, The generator will compute the clusters on-the-fly, meaning it will only compute the next cluster once you ask for it.
    clusters = dict(cluster_iterator) # dictiponary {'contignames',latent_vectors}
    ...
    # Split clusters file 
    path_clu='Clusters_2_split'+latents_path.split('Latents')[1].split('_latents.npz')[0]+'_S_new_clusters.tsv'
    ...
    bindir='./Clusters_2_split/Airways/bins/'+latents_path.split('/')[-1].split('_latents.npz')[0]
    print('\n Bin dir:',bindir,'\n')
...
clustering_aae_2(latents_path,dataset)
